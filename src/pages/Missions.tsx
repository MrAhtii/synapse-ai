...
import { useDailyMission, MISSION_TASKS, MISSION_BONUS_XP } from "../hooks/useDailyMission";
...
function getMotivation(completed: number, total: number): string {
  ...
  if (ratio <= 0.25) return "Great start! Every bit of learning counts — keep it up!";
    ...
    <PageContainer maxWidth="lg">
      ...
      <div className="mb-8 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Card className="flex items-center gap-3 !p-4">
          ...
          <div>
            <p className="text-xs font-medium text-foreground/50 uppercase tracking-wider">Progress</p>
            <p className="text-lg font-bold text-foreground">{isLoading ? "—" : `${progress}%`}</p>
          </div>
        </Card>
        ...
        <Card className="flex items-center gap-3 !p-4">
          ...
          <div>
            <p className="text-xs font-medium text-foreground/50 uppercase tracking-wider">Streak</p>
            <p className="text-lg font-bold text-foreground">
              {isDemo ? 7 : stats.learningStreak} day{isDemo ? "" : stats.learningStreak === 1 ? "" : "s"}
            </p>
          </div>
        </Card>
        ...
        <Card className="flex items-center gap-3 !p-4">
          ...
          <div>
            <p className="text-xs font-medium text-foreground/50 uppercase tracking-wider">Est. Time</p>
        ...
        <Card className="flex items-center gap-3 !p-4">
          ...
          <div>
            <p className="text-xs font-medium text-foreground/50 uppercase tracking-wider">XP to Earn</p>
            <p className="text-lg font-bold text-foreground">{xpTarget + (allDone ? 0 : MISSION_BONUS_XP)} XP</p>
          </div>
        </Card>
      </div>
      ...
      <div className="mb-8 space-y-3">
        <h2 className="font-heading text-lg font-bold text-foreground mb-1">Today&apos;s Tasks</h2>
        ...
        {MISSION_TASKS.map((def) => {
          ...
          return (
            <button
              ...
              } ${isDemo ? "opacity-60 cursor-default" : ""}`}
            >
              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-all duration-200 ${
                isCompleted ? "bg-emerald-500 text-white" : "bg-muted text-foreground/40"
              }`}>
                {isCompleted ? <CheckCircle2 size={18} /> : <Circle size={18} />}
              </span>

              <span className={`flex items-center gap-2.5 flex-1 text-base font-medium ${
                isCompleted ? "text-emerald-800 line-through decoration-emerald-400/50" : "text-foreground"
              }`}>
                <span className={isCompleted ? "text-emerald-500" : "text-foreground/50"}>
                  {def.key === "summary" ? <BookOpen size={18} /> : def.key === "flashcards" ? <Brain size={18} /> : def.key === "quiz" ? <ListChecks size={18} /> : <Star size={18} />}
                </span>
                {def.label}
              </span>

              <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-semibold ${
                isCompleted ? "bg-emerald-100 text-emerald-700" : "bg-muted text-foreground/50"
              }`}>
                +{def.xp} XP
              </span>
      ...
      <Card className="!p-6 sm:!p-8 mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-semibold text-foreground/60">Daily Progress</span>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground/70">
            Total XP Earned:{" "}
            <span className="text-foreground font-bold">{xpEarned}</span>
            <span className="text-foreground/40"> / {xpTarget + (allDone ? 0 : MISSION_BONUS_XP)}</span>
            {allDone && <span className="text-emerald-600 ml-1">+{MISSION_BONUS_XP} bonus</span>}
          </span>
        </div>
        ...
        <div className="mt-4 flex items-start gap-2 rounded-xl bg-accent/5 border border-accent/15 px-4 py-3">
          ...
          <p className="text-sm text-foreground/80 leading-relaxed">{getMotivation(completedCount, totalCount)}</p>
        </div>
      </Card>
